...
export const validateCreationParams = (owners: string[], threshold: number): void => {
  if (owners.length <= 0) throw new Error('Invalid owners: it must have at least one')
  if (threshold <= 0) throw new Error('Invalid threshold: it must be greater than or equal to 0')
  if (threshold > owners.length)
    throw new Error('Invalid threshold: it must be lower than or equal to owners length')
}

export const createGnosisSafeProxyFactoryContract = (
  proxyFactoryAddress: string,
  signer: Signer
): Contract => new Contract(proxyFactoryAddress, GnosisSafeProxyFactory.abi, signer)
...
}: SafeAccountConfiguration): string =>
  new utils.Interface(GnosisSafe.abi).encodeFunctionData('setup', [
    owners,
    threshold,
    to,
    data,
...
export const deployProxyFactory =
  (proxyFactory: Contract, safeSingletonAddress: string) =>
  (
    data: string,
